...
Popups.attach = function(context, selector, options) {
  $(selector, context).not('.popups-processed').each(function() {
    ...
    //Commented by Feroze on Dec 31 2011 to not display the text [Popup]
...
Popups.formSuccess = function(popup, data) {
  ...
  // If the response contain an error, set done to false.
  if (data.content.match(/class="[\w-\s]*?error[\w-\s]*?"/gm)) {
    var done = false;
  } else {
    var done = true;
  }
    ...
    if (popup.options.updateMethod === 'reload') { // Force a complete, non-ajax reload of the page.
      if (popup.options.updateSource === 'final') {
        ...
        if(data.messages == "") {
          ...
          //location.href = Drupal.settings.basePath + 'content/thank-you';
        }
      }
    }
